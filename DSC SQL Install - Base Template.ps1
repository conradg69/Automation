...
Configuration SQLInstall
{
    ...
    Node $AllNodes.where({ $_.Role -eq "SQLENGINE" }).NodeName 
    {
        ...
        Script Add_sp_WhoIsActiveToDBADatabase
        {
            GetScript =
            { 
                $Procedures = Get-DbaDbStoredProcedure -SqlInstance Lonpesssql01 -Database DBAToolKit -ExcludeSystemSp
                $Result =  ($Procedures.Name | where {$_ -eq 'sp_WhoIsActive'}) -eq 'sp_WhoIsActive';
                $Result
            }
            SetScript =
            {
                 Install-DbaWhoIsActive -SqlInstance Lonpesssql01 -Database DBAToolKit -LocalFile F:\SQLInstall\DBASetupScripts\who_is_active_v11_32.zip    
            }
            TestScript =
            {
                $Procedures = Get-DbaDbStoredProcedure -SqlInstance Lonpesssql01 -Database DBAToolKit -ExcludeSystemSp
        ...
        Script InstallFirstResponderKit
        {
            ...
            { 
                ...
                $Result = ((Get-DbaDbStoredProcedure -SqlInstance Lonpesssql01 -Database DBAToolKit -ExcludeSystemSp).name | 
            ...
            SetScript =
            {
                 Install-DbaFirstResponderKit -SqlInstance Lonpesssql01 -Database DBAToolKit -LocalFile F:\SQLInstall\DBASetupScripts\FirstResponderKit.zip   
            }
            TestScript =
            {
